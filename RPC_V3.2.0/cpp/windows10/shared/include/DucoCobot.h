...
namespace DucoRPC {
	...
	// 机器人移动过程中对控制柜的IO操作数据
	struct OP {
		char time_or_dist_1;       // 轨迹起始点触发类型, 0:不启用, 1:时间触发, 2:距离触发
		char trig_io_1;            // 轨迹触发控制柜IO的输出序号, 范围1-16
		bool trig_value_1;         // 轨迹触发控制柜IO的电平高低, false:低电平, true:高电平
		double trig_time_1;        // 当time_or_dist_1为1时, 代表轨迹运行多少时间长度触发IO,单位: ms
		double trig_dist_1;        // 当time_or_dist_1为2时, 代表轨迹运行多少距离长度触发IO,单位: m
		std::string trig_event_1;  // 轨迹触发的用户自定义事件名称
		char time_or_dist_2;       // 轨迹结束点触发类型, 0:不启用, 1:时间触发, 2:距离触发
		char trig_io_2;            // 轨迹触发控制柜IO的输出序号, 范围1-16
		bool trig_value_2;         // 轨迹触发控制柜IO的电平高低, false:低电平, true:高电平
		double trig_time_2;        // 当time_or_dist_2为1时, 当trig_time_2 >= 0时, 代表轨迹运行剩余多少时间长度触发IO,单位: ms; 当trig_time_2 < 0时, 代表代表轨迹运行结束后多少时间长度触发IO
		double trig_dist_2;        // 当time_or_dist_2为2时, 当trig_ dist _2 >= 0时, 代表轨迹运行剩余多少距离长度触发IO,单位: m;当trig_ dist _2 < 0时, 代表代表轨迹运行结束后多少距离长度触发IO
	...
	struct RealTimeData
	{
		std::vector<double>  joint_pos_cmd;               // 关节位置
		std::vector<double>  joint_vel_cmd;               // 关节速度
		std::vector<double>  joint_torq_cmd;              // 关节力矩  预留
		std::vector<double>  cart_pos_tool_wobj_cmd;      // 笛卡尔位置
		std::vector<double>  cart_vel_tool_wobj_cmd;      // 笛卡尔速度
		std::vector<double>  cart_ft_cmd;                 // 笛卡尔力和力矩  预留
		bool status;                                      // 控制指令刷新状态, 更新机器人控制指令是给true
	};


	class DLL_EXPORT_IMPORT DucoCobot
	{
	public:
		/**
		 * @brief 创建机器人远程连接的实例
		 ...
		 * @param active 是否启用, 默认为true
		 * @return 返回当前任务结束时的状态
		 */
		int32_t set_wobj_offset(const std::vector<double>& wobj, bool active = true);
		/**
		 ...
		 * @param r 关节融合半径, 单位: 系统设定最大融合半径的百分比%, 值为 0, 表示无融合, 取值范围[0,50)
		 * @param block 是否阻塞, 如果为false表示非阻塞指令, 指令会立即返回
		 * @param op 可缺省参数
		 ...
		 */
		int32_t movej(const std::vector<double>& joints_list, double v, double a, double r, bool block, const OP& op = _op, bool def_acc = true);
		/**
		 * @brief 控制机械臂从当前状态, 按照关节运动的方式移动到目标关节角状态
		 * @param joints_list 1-6关节的目标关节角度, 单位: rad
		 ...
		 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
		 */
		int32_t movej2(const std::vector<double>& joints_list, double v, double a, double r, bool block, const OP& op = _op, bool def_acc = true);
		/**
		 ...
		 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
		 */
		int32_t movej_pose(const std::vector<double>& p, double v, double a, double r, const std::vector<double>& q_near, const std::string& tool, const std::string& wobj, bool block, const OP& op = _op, bool def_acc = true);
		/**
		 * @brief 控制机械臂从当前状态, 按照关节运动的方式移动到末端目标位置
		 * @param p 对应末端的位姿, 位置单位: m, 姿态以Rx、Ry、Rz表示, 单位: rad
		 * @param v 关节角速度, 单位: rad/s
		 ...
		 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
		 */
		int32_t movej_pose2(const std::vector<double>& p, double v, double a, double r, const std::vector<double>& q_near, const std::string& tool, const std::string& wobj, bool block, const OP& op = _op, bool def_acc = true);
		/**
		 ...
		 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
		 */
		int32_t movel(const std::vector<double>& p, double v, double a, double r, const std::vector<double>& q_near, const std::string& tool = "default", const std::string& wobj = "default", bool block = true, const OP& op = _op, bool def_acc = true);
		/**
		 ...
		 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
		 */
		int32_t movec(const std::vector<double>& p1, const std::vector<double>& p2, double v, double a, double r, int mode, const std::vector<double>& q_near, const std::string& tool, const std::string& wobj, bool block, const OP& op = _op, bool def_acc = true);
		/**
		 ...
		 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
		 */
		int32_t move_circle(const std::vector<double>& p1, const std::vector<double>& p2, double v, double a, double r, bool mode, const std::vector<double>& q_near, const std::string& tool, const std::string& wobj, bool block, const OP& op = _op, bool def_acc = true);
		/**
		 ...
		 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
		 */
		int32_t tcp_move(const std::vector<double>& pose_offset, double v, double a, double r, const std::string& tool, bool block, const OP& op = _op, bool def_acc = true);
		/**
		 ...
		 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
		 */
		int32_t tcp_move_2p(const std::vector<double>& p1, const std::vector<double>& p2, double v, double a, double r, const std::string& tool, const std::string& wobj, bool block, const OP& op = _op, bool def_acc = true);
		/**
		 ...
		 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
		 */
		int32_t spline(const std::vector<std::vector<double> >& pose_list, double v, double a, const std::string& tool, const std::string& wobj, bool block, const OP& op = _op, double r = 0, bool def_acc = true);
		/**
		 ...
		 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
		 */
		int32_t servoj(const std::vector<double>& joints_list, double v, double a, bool block = false, double kp = 200, double kd = 25);
		/**
		 ...
		 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
		 */
		int32_t servoj_pose(const std::vector<double>& pose_list, double v, double a, const std::vector<double>& q_near, const std::string& tool, const std::string& wobj, bool block = false, double kp = 200, double kd = 25);
		/**
		 ...
		 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
		 */
		int32_t servo_tcp(const std::vector<double>& pose_offset, double v, double a, const std::string& tool, bool block = false, double kp = 200, double kd = 25);
		/**
		 * @brief 控制机器人进入牵引示教模式
		 * @param block 是否阻塞, 如果为false表示非阻塞指令, 指令会立即返回
		 * @return 阻塞执行代表任务结束时状态, 非阻塞执行代表任务的ID
		 ...
		 * @return 当前任务的id信息
		 */
		int32_t fc_config(const std::vector<bool>& direction, const std::vector<double>& ref_ft, const std::vector<double>& damp, const std::vector<double>& max_vel, const std::vector<double>& number_list, const std::string& toolname, const std::string& wobjname, int32_t value);
		/**
		 ...
		 * @return 阻塞执行代表任务结束时状态, 非阻塞执行代表任务的ID
		 */
		int32_t move_spiral(const std::vector<double>& p1, const std::vector<double>& p2, double rev, double len, double r, int32_t mode, double v, double a, const std::vector<double>& q_near, const std::string& tool, const std::string& wobj, bool block, const OP& op = _op, bool def_acc = true);
		/**
		 ...
		 * @return 任务结束时状态
		 */
		int32_t combine_motion_config(int32_t type, int32_t ref_plane, int32_t fq, int32_t amp, int32_t el_offset, int32_t az_offset, int32_t up_height, const std::vector<int32_t>& time, const std::vector<OP>& op_list);
		/**
		 * @brief 开启复合运动
		 * @return 任务结束时状态
		 */
		 ...
		 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
		 */
		int32_t movej2_eaxis(const std::vector<double>& joints_list, double v, double a, double rad, const std::string& scheme_name, const std::vector<double>& epose, double eaxis_v, bool block, const OP& op = _op, bool def_acc = true);
		/**
		 ...
		 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
		 */
		int32_t movej2_pose_eaxis(const std::vector<double>& p, double v, double a, double rad, const std::vector<double>& qnear, const std::string& tool, const std::string& wobj, const std::string& scheme_name, const std::vector<double>& epose, double eaxis_v, bool block, const OP& op = _op, bool def_acc = true);
		/**
		 ...
		 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
		 */
		int32_t movel_eaxis(const std::vector<double>& p, double v, double a, double rad, const std::vector<double>& qnear, const std::string& tool, const std::string& wobj, const std::string& scheme_name, const std::vector<double>& epose, double eaxis_v, bool block, const OP& op = _op, bool def_acc = true);
		/**
		 ...
		 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
		 */
		int32_t movec_eaxis(const std::vector<double>& p1, const std::vector<double>& p2, double v, double a, double rad, const std::vector<double>& qnear, const std::string& tool, const std::string& wobj, const std::string& scheme_name, const std::vector<double>& epose, double eaxis_v, bool block, const OP& op = _op, bool def_acc = true);
		/**
		 * @brief 可达性检查
		 ...
		 * @return 任务结束时的状态
		 */
		int32_t set_hand_teach_parameter(const int32_t space, const int32_t joint_scale, const int32_t cart_scale, const int32_t coord_type, const std::vector<bool>& direction);
		/**
		 * @brief 示教器按键的jog类型
		 ...
		 * @return 任务结束时的状态
		 */
		int32_t set_blend_ahead(const int32_t per);
		/**
		 * @brief 开启实时控制模式
		 ...
		 * @return 任务结束时的状态
		 */
		int32_t start_realtime_mode(int32_t mode, double filter_bandwidth = 100, double com_lost_time = 0.02);
		/**
		 * @brief 结束实时控制模式
		 * @return 任务结束时的状态
		 */
