...
class DucoCtrl():
    ...
    def DucoMoveTcp(self,PosOffset,vel,acc,toolname):
        '''
            * Function:     DucoMoveTcp
            * Description:  控制Duco沿末端坐标系进行直线运动
            * Inputs:       
                            PosOffset: 以末端工具为基准的偏移量 [x, y, z, rx, ry, rz]，位置单位: m，姿态范围[-2*pi, 2*pi]，单位rad
                            vel: 末端运动速度，范围[0, 5]，单位(m/s)
                            acc: 末端加速度，范围[0, 5]，单位(m/s²)
                            ToolName: 工具坐标系的名字
            * Outputs:        
            ...
            * Notes:
        '''
        self.DucoRobot.tcp_move(pose_offset=PosOffset,v=vel,a=acc,r=0,tool=toolname,block=False)
        while (True):
            DucoState = self.IsDucoMoving()
                # isMoving = self.DucoRobot.robotmoving()
                # if isMoving == False:
